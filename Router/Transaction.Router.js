...
transactionRouter.get('/initialize-database', async (req, res) => {
  ...
  transactionRouter.get('/transactions', paginateResults(TransactionModel), async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const perPage = parseInt(req.query.perPage) || 10;
    ...
    try {
      ...
      const transactions = await TransactionModel.find(query).limit(perPage).skip(startIndex).exec();
