...
statisticRouter.get('/statistics', async (req, res) => {
    try {
        ...
        const totalSoldItems = await TransactionModel.countDocuments({ month: selectedMonth, sold: true });
        const totalNotSoldItems = await TransactionModel.countDocuments({ month: selectedMonth, sold: false });
